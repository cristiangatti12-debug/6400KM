...
function buildInitialStops(initial?: PostFormInitial): Stop[] {
  const itin = initial?.itinerary;
  if (!itin || itin.destinations.length === 0) {
    return [{ text: "", status: "idle" }];
  }
  const points = itin.stop_points ?? [];
  return itin.destinations.map((name) => {
    ...
    return hit
      ...
      : { text: name, status: "idle" as const };
  });
}
...
export function PostForm({
  ...
  const [itTitle, setItTitle] = useState(initial?.itinerary?.title ?? "");
  const [stops, setStops] = useState<Stop[]>(buildInitialStops(initial));
  const [days, setDays] = useState(
    initial?.itinerary?.days ? String(initial.itinerary.days) : ""
  );
  const [interests, setInterests] = useState<string[]>(
    initial?.itinerary?.interest_tags ?? []
  );
  const [budget, setBudget] = useState(initial?.itinerary?.budget_level ?? "");

  const [error, setError] = useState<string | null>(null);
  const [saving, setSaving] = useState(false);

  const totalPhotos = existingMedia.length + files.length;
  const located: StopPoint[] = stops
  ...
  async function locateStop(i: number) {
    ...
    setStops((prev) =>
      prev.map((s, idx) =>
        idx === i
          ? hit
            ? { ...s, lat: hit.lat, lng: hit.lng, label: hit.label, status: "ok" }
            : { ...s, status: "notfound" }
          : s
      )
    );
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      // Fill in coordinates for any stop that hasn't been looked up yet.
      const resolved = [...clean];
      for (let i = 0; i < resolved.length; i++) {
        if (resolved[i].lat == null) {
          const hit = await geocode(resolved[i].text.trim());
          if (hit) resolved[i] = { ...resolved[i], lat: hit.lat, lng: hit.lng };
        }
      }

      // Compress + upload any new photos in parallel (keeps their order).
      ...
      // Update the existing itinerary, or create one.
      let itineraryId = initial?.itineraryId ?? null;
      ...
      if (isEdit && initial) {
        const { error: postErr } = await supabase
          .from("posts")
          .update(postFields)
          .eq("id", initial.postId);
        if (postErr) throw postErr;

        // Clean up photos the user removed.
        const dropped = (initial.media ?? [])
          .filter((m) => !media.some((k) => k.url === m.url))
          .map((m) => m.url.split("/post-media/")[1])
          .filter(Boolean);
        if (dropped.length) {
          await supabase.storage.from("post-media").remove(dropped);
        }

        router.push(`/post/${initial.postId}`);
      } else {
        const { error: postErr } = await supabase
          .from("posts")
          .insert({ author_id: userId, ...postFields });
        if (postErr) throw postErr;
        router.push("/profile");
      }
      router.refresh();
        ...
        <div className="flex flex-col gap-3">
          ...
          {stops.map((stop, i) => (
            <div key={i} className="flex flex-col gap-1">
              ...
              {stop.status === "idle" && stop.text.trim() && (
                <span className="text-xs text-muted-foreground">
                  Tap outside the box to place it on the map.
                </span>
              )}
